...
func matchPath(t *testing.T, tree *enroute.Tree, path string, expect string) error {
	t.Helper()
	...
	actual := match.String()
	return diff.String(actual, expect)
}
		...
		for _, route := range routes {
			...
			for _, request := range route.Requests {
				t.Run(route.Route, func(t *testing.T) {
					t.Helper()
					if err := matchPath(t, tree, request.Path, request.Expect); err != nil {
						for i, r := range routes {
							fmt.Println(i, ".", r.Route)
						}
						t.Fatal(err)
...
func TestHandlePrecedence(t *testing.T) {
	tree := enroute.New()
	noErr(t, tree.Insert("/{digits|[0-9]+}", "/{digits|[0-9]+}"))
	noErr(t, tree.Insert("/{public*}", "/{public*}"))
	noErr(t, tree.Insert("/", "/"))
}

func TestHandleWildcardOptional(t *testing.T) {
	tree := enroute.New()
	noErr(t, tree.Insert("/", "/"))
	noErr(t, tree.Insert("/{digits|[0-9]+}", "/{digits|[0-9]+}"))
	noErr(t, tree.Insert("/{public*}", "/{public*}"))
	...
	match, err := tree.Match("/a")
	noErr(t, err)
	if match.String() != "/{public?} public=a" {
		t.Fatalf("expected /{public?} public=a, got %s", match.String())
	}
}
